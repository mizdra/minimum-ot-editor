...
int cursor_move_by_index(EDITOR* editor, int n) {
  if (n < 0) {
    ERROR("cursor index must be 0 or above: (n = %d)", n);
    return 0;
  }
...
int cursor_move_by_position(EDITOR* editor, int row, int column) {
  if (row < 1 || column < 1) {  // row が小さすぎる場合
    ERROR("row is too small: (row, column) = (%d, %d)", row, column);
    return 0;
  }
  if (column < 1) {  // column が小さすぎる場合
    ERROR("column is too small: (row, column) = (%d, %d)", row, column);
    return 0;
  }
  ...
  while (r < row) {
    ...
    if (!row_head) {  // row が大きすぎる場合
      ERROR("row is too big: (row, column) = (%d, %d)", row, column);
      return 0;
    }
    r++;
  }
  if (column > get_column_count(editor, row)) {  // column が大きすぎる場合
    ERROR("column is too big: (row, column) = (%d, %d)", row, column);
    return 0;
  }
